...
pub type RigidBodies<'a, N> = Map<Iter<'a, Entry<usize, RigidBodyHandle<N>>>,
                                  fn(&'a Entry<usize, RigidBodyHandle<N>>) -> &'a RigidBodyHandle<N>>;

/// An iterator visiting sensors.
pub type Sensors<'a, N> = Map<Iter<'a, Entry<usize, SensorHandle<N>>>,
...
pub struct World<N: Real> {
    ...
    sleep: Arc<RwLock<ActivationManager<N>>>, // FIXME: avoid sharing (needed for the contact signal handler)
...
impl<N: Real> World<N> {
    ...
    pub fn new() -> World<N> {
        ...
        // Deactivation
        let sleep = Arc::new(RwLock::new(ActivationManager::new(na::convert(0.01f64))));
        ...
        cworld.register_contact_handler("__nphysics_internal_ObjectActivationOnContactHandler",
                                        handler);
    ...
    pub fn step(&mut self, dt: N) {
        for e in self.rigid_bodies.elements_mut().iter_mut() {
            let mut rb = e.value.write().unwrap();

            if rb.is_active() {
                self.forces.update(dt.clone(), &mut *rb);
                self.integrator.update(dt.clone(), &mut *rb);
                self.cworld
                    .deferred_set_position(WorldObject::rigid_body_uid(&e.value),
                                           rb.position().clone());
            }
        }

        for e in self.sensors.elements_mut().iter_mut() {
            let sensor = e.value.write().unwrap();

            if let Some(rb) = sensor.parent() {
                if rb.read().unwrap().is_active() {
        ...
        self.joints.update(&mut *self.sleep.write().unwrap());
        self.sleep
            .write()
            .unwrap()
            .update(&mut self.cworld, &self.joints, &self.rigid_bodies);
        ...
        for (b1, b2, c) in self.cworld.contacts() {
            if let (&WorldObject::RigidBody(ref rb1), &WorldObject::RigidBody(ref rb2)) =
                (&b1.data, &b2.data) {
                if rb1.read().unwrap().is_active() || rb2.read().unwrap().is_active() {
                    let m1 = rb1.read().unwrap().margin();
                    let m2 = rb2.read().unwrap().margin();
    ...
    pub fn add_sensor(&mut self, sensor: Sensor<N>) -> SensorHandle<N> {
        ...
        let handle = Arc::new(RwLock::new(sensor));
        let uid = &*handle as *const RwLock<Sensor<N>> as usize;
    ...
    pub fn remove_rigid_body(&mut self, rb: &RigidBodyHandle<N>) {
        ...
        self.cworld.perform_additions_removals_and_broad_phase();
        self.joints.remove(rb, &mut *self.sleep.write().unwrap());
        self.ccd.remove_ccd_from(rb);
        self.rigid_bodies.remove(&uid);
        rb.write().unwrap().delete();
    }
    ...
    pub fn add_ccd_to(&mut self,
        ...
        self.ccd
            .add_ccd_to(body.clone(), motion_thresold, trigger_sensors)
    }

    /// Adds a ball-in-socket joint to the world.
    pub fn add_ball_in_socket(&mut self, joint: BallInSocket<N>) -> Arc<RwLock<BallInSocket<N>>> {
        let res = Arc::new(RwLock::new(joint));

        self.joints
            .add_ball_in_socket(res.clone(), &mut *self.sleep.write().unwrap());

        res
    }

    /// Removes a ball-in-socket joint from the world.
    pub fn remove_ball_in_socket(&mut self, joint: &Arc<RwLock<BallInSocket<N>>>) {
        self.joints
            .remove_ball_in_socket(joint, &mut *self.sleep.write().unwrap())
    }

    /// Adds a fixed joint to the world.
    pub fn add_fixed(&mut self, joint: Fixed<N>) -> Arc<RwLock<Fixed<N>>> {
        let res = Arc::new(RwLock::new(joint));

        self.joints
            .add_fixed(res.clone(), &mut *self.sleep.write().unwrap());

        res
    }

    /// Removes a fixed joint from the world.
    pub fn remove_fixed(&mut self, joint: &Arc<RwLock<Fixed<N>>>) {
        self.joints
            .remove_joint(joint, &mut *self.sleep.write().unwrap())
    }
    ...
    pub fn register_broad_phase_pair_filter<F>(&mut self, name: &str, filter: F)
        where F: BroadPhasePairFilter<Point<N>, Isometry<N>, WorldObject<N>> + 'static + Send + Sync
    {
    ...
    pub fn register_contact_handler<H>(&mut self, name: &str, handler: H)
        where H: ContactHandler<Point<N>, Isometry<N>, WorldObject<N>> + 'static + Send + Sync
    {
    ...
    pub fn register_proximity_handler<H>(&mut self, name: &str, handler: H)
        where H: ProximityHandler<Point<N>, Isometry<N>, WorldObject<N>> + 'static + Send + Sync
    {
...
struct ObjectActivationOnContactHandler<N: Real> {
    sleep: Arc<RwLock<ActivationManager<N>>>,
}
...
impl<N: Real> ContactHandler<Point<N>, Isometry<N>, WorldObject<N>>
    ...
    fn handle_contact_stopped(&mut self,
        ...
        if let (&WorldObject::RigidBody(ref rb1), &WorldObject::RigidBody(ref rb2)) =
            (&obj1.data, &obj2.data) {
            self.sleep.write().unwrap().deferred_activate(rb1);
            self.sleep.write().unwrap().deferred_activate(rb2);
        }
    }
}
