...
impl<N: Real> AccumulatedImpulseSolver<N> {
    ...
    fn do_solve(&mut self,
        ...
        self.resize_buffers(num_restitution_equations + num_joint_equations,
                            num_friction_equations);
        ...
        for (i, (_, &(ci, imp))) in self.cache.hash().iter().enumerate() {
            match constraints[ci] {
                Constraint::RBRB(ref rb1, ref rb2, ref c) => {
                    contact_equation::fill_second_order_equation(dt.clone(),
                                                                 c,
                                                                 &*rb1.read().unwrap(),
                                                                 &*rb2.read().unwrap(),
        ...
        for i in joints.iter() {
            ...
            match constraints[*i] {
                ...
                Constraint::Fixed(ref f) => {
                    fixed_equation::fill_second_order_equation(dt.clone(),
                                                               &*f.read().unwrap(),
                                                               &mut self.restitution_constraints
                                                                   [joint_offset..nconstraints], // XXX
                                                               &self.correction);
        ...
        pgs::projected_gauss_seidel_solve(&mut self.restitution_constraints[..],
                                          &mut self.friction_constraints[..],
                                          &mut self.mj_lambda[..],
                                          bodies.len(),
                                          self.num_second_order_iter,
...
impl<N: Real> Solver<N, Constraint<N>> for AccumulatedImpulseSolver<N> {
    fn solve(&mut self, dt: N, constraints: &[Constraint<N>]) {
        ...
        if constraints.len() != 0 {
            ...
            for (i, cstr) in constraints.iter().enumerate() {
                match *cstr {
                    Constraint::RBRB(ref a, ref b, ref c) => {
                        self.cache.insert(i,
                                          &**a as *const RwLock<RigidBody<N>> as usize,
                                          &**b as *const RwLock<RigidBody<N>> as usize,
            ...
            for c in constraints.iter() {
                match *c {
                    Constraint::RBRB(ref a, ref b, _) => {
                        a.write().unwrap().set_index(-2);
                        b.write().unwrap().set_index(-2)
                    }
                    Constraint::BallInSocket(ref bis) => {
                        let bbis = bis.read().unwrap();
                        match bbis.anchor1().body {
                            Some(ref b) => b.write().unwrap().set_index(-2),
                            None => {}
                        };
            ...
            fn set_body_index<N: Real>(a: &Arc<RwLock<RigidBody<N>>>,
                                       bodies: &mut Vec<Arc<RwLock<RigidBody<N>>>>,
                                       id: &mut isize) {
                let mut ba = a.write().unwrap();
                if ba.index() == -2 {
                    if ba.can_move() {
                        ba.set_index(*id);
                        bodies.push(a.clone());
