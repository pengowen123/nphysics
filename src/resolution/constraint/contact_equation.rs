...
pub struct CorrectionParameters<N: Real> {
    pub corr_mode: CorrectionMode<N>,
    pub joint_corr: N,
    pub rest_eps: N,
}

pub fn reinit_to_first_order_equation<N: Real>(
    dt: N,
    ...
    if coll.depth >= correction.corr_mode.min_depth_for_pos_corr() {
        constraint.objective = correction.corr_mode.pos_corr_factor() *
            coll.depth.max(na::zero()) / dt;
    } else {
...
pub fn fill_second_order_equation<N: Real>(
    ...
    Vector::orthonormal_subspace_basis(&[coll.normal], |friction_axis| {
...
fn fill_velocity_constraint<N: Real>(
    dt: N,
    normal: Vector<N>,
    center: Point<N>,
    restitution: N,
    ...
    let rot_axis1 = (center - *rb1.center_of_mass()).gcross(&-normal);
    let rot_axis2 = (center - *rb2.center_of_mass()).gcross(&normal);
    ...
    if false {
        ...
        constraint.objective = constraint.objective +
            depth * correction.corr_mode.vel_corr_factor() / dt
    }

    // for warm-starting
    constraint.impulse = if depth < na::zero() {
        na::zero()
    } else {
        initial_impulse
    };
...
pub fn relative_velocity<N: Real>(
    rb1: &Option<&RigidBody<N>>,
    rb2: &Option<&RigidBody<N>>,
    normal: &Vector<N>,
    rot_axis1: &Orientation<N>,
