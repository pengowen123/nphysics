...
impl<N: Real> ActivationManager<N> {
    ...
    pub fn update(
        ...
        if bodies.len() > self.ufind.len() {
            ...
            self.ufind.extend(
                iter::repeat(UnionFindSet::new(0)).take(to_add),
            );
        ...
        for e in joints.joints().elements().iter() {
            match e.value {
                ...
                Constraint::BallInSocket(ref b) => {
                    match (
                        b.borrow().anchor1().body.as_ref(),
                        b.borrow().anchor2().body.as_ref(),
                    ) {
                ...
                Constraint::Fixed(ref f) => {
                    match (
                        f.borrow().anchor1().body.as_ref(),
                        f.borrow().anchor2().body.as_ref(),
                    ) {
                        (Some(b1), Some(b2)) => make_union(b1, b2, &mut self.ufind[..]),
                        _ => {}
                    }
