...
impl<N: Real> ActivationManager<N> {
    ...
    pub fn deferred_activate(&mut self, b: &RigidBodyHandle<N>) {
        if b.read().unwrap().can_move() && !b.read().unwrap().is_active() {
        ...
        match b.deactivation_threshold() {
            Some(threshold) => {
                ...
                let new_energy = (_1 - self.mix_factor) * b.activation_state().energy() +
                                 self.mix_factor *
                                 (na::norm_squared(&b.lin_vel()) + na::norm_squared(&b.ang_vel()));
    ...
    pub fn update(&mut self,
        ...
        for (i, b) in bodies.elements().iter().enumerate() {
            let mut b = b.value.write().unwrap();
        ...
        if bodies.len() > self.ufind.len() {
            ...
            self.ufind
                .extend(iter::repeat(UnionFindSet::new(0)).take(to_add));
        ...
        // Run the union-find.
        fn make_union<N: Real>(b1: &RigidBodyHandle<N>,
                               b2: &RigidBodyHandle<N>,
                               ufs: &mut [UnionFindSet]) {
            let rb1 = b1.read().unwrap();
            let rb2 = b2.read().unwrap();
        ...
        for e in joints.joints().elements().iter() {
            match e.value {
                ...
                Constraint::BallInSocket(ref b) => {
                    match (b.read().unwrap().anchor1().body.as_ref(),
                           b.read().unwrap().anchor2().body.as_ref()) {
                        (Some(b1), Some(b2)) => make_union(b1, b2, &mut self.ufind[..]),
                        _ => {}
                    }
                }
                Constraint::Fixed(ref f) => {
                    match (f.read().unwrap().anchor1().body.as_ref(),
                           f.read().unwrap().anchor2().body.as_ref()) {
                        (Some(b1), Some(b2)) => make_union(b1, b2, &mut self.ufind[..]),
                        _ => {}
                    }
                }
        ...
        for i in 0usize..self.ufind.len() {
            ...
            let b = bodies.elements()[i].value.read().unwrap();

            self.can_deactivate[root] = match b.deactivation_threshold() {
                Some(threshold) => {
        ...
        for i in 0usize..self.ufind.len() {
            let root = union_find::find(i, &mut self.ufind[..]);
            let mut b = bodies.elements()[i].value.write().unwrap();
